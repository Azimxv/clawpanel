...
def _build_corp_exit_outbound() -> dict | None:
    ...
    fp = os.environ.get("CORP_EXIT_FINGERPRINT", "firefox")
...
def build_xray_config(clients: List[Dict], node_address: str,
    ...
    direct_inbounds = ["VLESS-XHTTP-DIRECT"]  # HY2 disabled (upstream xray-core has no finalmask)
    ...
    # HY2 inbounds disabled — upstream xray-core 26.5.9 has no finalmask/xdns/xicmp.
    # Re-enable when xray-hy custom build with finalmask is restored.
    # inbounds.extend([
    #     _build_hy2_inbound("HY2-XDNS", 53, ...),
    #     _build_hy2_inbound("HY2-XICMP", 9053, ...),
    # ])
...
def _hy_direct_addr(addr: str) -> str:
    """Return node address for HY2 (standalone: no CDN, no hy- prefix needed)."""
    return addr
    ...
    for node in nodes:
        ...
        if "exit" in enabled:
            links.append(
                ...
                f"&sni={addr}&alpn=h2,http/1.1&fp=firefox&mode=packet-up"
                f"&path={_urlquote(xhttp_path, safe='')}"
                f"&extra=%7B%22scMaxEachPostBytes%22%3A1000000%2C%22scMinPostsIntervalMs%22%3A30%2C%22xmux%22%3A%7B%22maxConcurrency%22%3A16%2C%22hMaxRequestTimes%22%3A900%2C%22hMaxReusableSecs%22%3A3000%2C%22hKeepAlivePeriod%22%3A15%7D%7D"
                f"#{username}-exit"
            )
        if "direct" in enabled:
            links.append(
        ...
        if "h3-exit" in enabled:
            links.append(
                ...
                f"?encryption=none&type=xhttp&security=tls"
                f"&sni={addr}&alpn=h3&fp=firefox&mode=packet-up"
                f"&path={_urlquote(xhttp_path, safe='')}"
                f"&extra=%7B%22scMaxEachPostBytes%22%3A1000000%2C%22scMinPostsIntervalMs%22%3A30%2C%22xmux%22%3A%7B%22maxConcurrency%22%3A16%2C%22hMaxRequestTimes%22%3A900%2C%22hMaxReusableSecs%22%3A3000%2C%22hKeepAlivePeriod%22%3A15%7D%7D"
                f"#{username}-h3-exit"
            )
